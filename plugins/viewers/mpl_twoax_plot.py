...
class MPLTwoAxisPlot(Viewer):
  ...
  def update(self):
    ...
    if lines:
      y_exprs = self.get_y_exprs()
      if len(y_exprs) > len(lines):
        for i in range(len(y_exprs) - len(lines)):
          axes.plot([0], [0])
        lines = axes.get_lines()
  
      max_xs = max_ys = min_xs = min_ys = 0
  
      for n, y_expr in enumerate(y_exprs):
        first = 0
        last = None
  ...
  def get_x_exprs(self):
    return self.x_expressions.split(',')

  def get_y_exprs(self):
    return self.y_expressions.split(',')
